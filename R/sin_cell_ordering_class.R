...
#' @param save_path The file path to save the RDA file containing the information on
#'          gene characteristics and the cell ordering times. Default is NULL in which
#'          case, it will not save output
...
sin_cell_ordering_class <- function(cycle_data, celltime_levels,
                                    num_shuffle = NULL, save_path=NULL,
                                    ...
                                    n_cores=1)
{
  ...
  if (is.null(num_shuffle)) num_shuffle <- maxiter
  ...
  while (TRUE) {
    estimates <- sin_cell_ordering_iter(cycle_data=cycle_data,
                                        ...
                                        fix.phase=fix.phase, phase_in=phase_in)
    ...
    loglik_iter <- estimates$loglik_iter
    loglik_iter_vec <- c(loglik_iter_vec, loglik_iter)
    ...
    # for one iteration, shuffle cell times num_shuffle times
    for(index in 1:num_shuffle) {
      ...
      loglik_iter <- fun$loglik_iter

      if (verbose == TRUE) {
        cat("The loglikelihood after shuffle", index, "is:", loglik_iter,"\n")
      }
    }
